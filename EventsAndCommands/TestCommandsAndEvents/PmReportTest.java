...
public class PmReportTest extends ListenerAdapter {
    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent event) {
        Runnable timeOut = () -> event.getChannel().sendMessage("Your report has timed out because you haven't replied in 5 minutes").queue();
                            ...
                            waiter.waitForEvent(event.getClass(), evidence -> eventAuthor.equals(evidence.getAuthor()), evidenceMessage -> {
                                ...
                                if(!evidenceMessage.getMessage().getAttachments().isEmpty()) {
                                    if(evidenceMessage.getMessage().getAttachments().get(0).isImage()) {
                                        embedBuilder.setImage(evidenceMessage.getMessage().getAttachments().get(0).getUrl());
                                    }
                                }
